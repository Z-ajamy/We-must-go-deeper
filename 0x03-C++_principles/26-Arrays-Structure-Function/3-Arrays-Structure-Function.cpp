...
void in_pereson_fun (struct strinfo pereson[2])
{
    in_fun (pereson[0]);  // عند استدعاء داله فان الاستدعاء يكون بالاسم فقط سواء في الباراميتر او في الداله نفسها وليس بكل التعريف ولكن عند تعريف الدالة نعرف في الباراميتر كل المتغير
    in_fun (pereson[1]);
    in_fun (pereson[2]);
}

void out_pereson_fun (struct strinfo pereson[2])
{
    out_fun(pereson[0]);
    out_fun(pereson[1]);
    out_fun(pereson[2]);
...
int main ()
{
    ...
    in_pereson_fun (pereson);
    out_pereson_fun (pereson);
